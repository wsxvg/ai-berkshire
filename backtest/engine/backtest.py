...
def score_4433(fund_code, cutoff_date, fund_charts):
    ...
    for name, (days, _) in periods.items():
        if n > days:
            start_nav = navs[-days]
            rets[name] = (cur_nav - start_nav) / start_nav * 100 if start_nav != 0 else 0
        else:
            rets[name] = None
    ...
    if cutoff_date not in _4433_RANK_CACHE:
        ...
        for code, pts_all in fund_charts.items():
            ...
            for name, (days, _) in periods.items():
                if len(v_nav) > days:
                    sv_nav = v_nav[-days]
                    r = (cv_nav - sv_nav) / sv_nav * 100 if sv_nav != 0 else 0
...
def score_fund_backtest(fund_code, fund_name, charts, perf_data, rules, mgr,
    ...
    if alloc_data:
        ...
        if top_stocks:
            top10_pct = sum(float(str(s.get("ratio", "0")).replace("%","")) for s in top_stocks[:10])
            if top10_pct > 70:
...
class Portfolio:
    ...
    def sell(self, code, amount, price=1.0, day_str="", sell_reason="", force_sell=False):
        ...
        if amount <= 0:
            ...
            return True
        # 部分卖出
        shares_to_sell = amount / price if price > 0 else 0
...
def run_backtest(config, clear_cache=True):
    ...
    for idx, day in enumerate(backtest_dates):
        ...
        # V6B: 4433 buy filter - only buy funds passing 4433 rank check
        _require_4433 = config.get('require_4433_pass', 0)
        if _require_4433 > 0 and candidates:
            filtered = []
            for c in candidates:
                ...
                if not _cpts:
                    filtered.append(c)
                    continue
                _, _p4433 = score_4433(c['code'], cutoff_full, fund_charts)
                if _p4433 >= _require_4433:
                    filtered.append(c)
            candidates = filtered
            if idx % 20 == 0 and len(filtered) == 0:
                print(f'  4433_filter: require>={_require_4433}, 0 candidates passed')
        ...
        if _rel_strength and candidates and _bm_nav_values and len(_bm_nav_values) >= 20:
            _bm_ret_20 = _bm_nav_values[-1] / _bm_nav_values[-20] - 1 if _bm_nav_values[-20] != 0 else 0
            filtered = []
            for c in candidates:
                _cpts = fund_charts.get(c["code"], [])
                _cvalid = _bisect_valid(_cpts, cutoff_full)
                if len(_cvalid) >= 20:
                    _cnavs = [(100 + _float(p.get("yAxis", 0))) / 100 for p in _cvalid]
                    _c_ret = _cnavs[-1] / _cnavs[-20] - 1 if _cnavs[-20] != 0 else 0
        ...
        _fund_drop_buy = config.get("fund_drop_buy", 0)
        _fund_drop_days = config.get("fund_drop_days", 5)
        if _fund_drop_buy > 0 and candidates:
            filtered = []
            for c in candidates:
                _cpts = fund_charts.get(c["code"], [])
                _cvalid = _bisect_valid(_cpts, cutoff_full)
                if len(_cvalid) >= _fund_drop_days + 1:
                    _cnavs = [(100 + _float(p.get("yAxis", 0))) / 100 for p in _cvalid]
                    _prev_nav = _cnavs[-_fund_drop_days - 1]
                    if _prev_nav > 0:
                        _fund_ret = _cnavs[-1] / _prev_nav - 1
                        if _fund_ret < -_fund_drop_buy:
                            filtered.append(c)
            candidates = filtered
        ...
        # 重置每日标记（_dd_reduced_today 等）
        for _h_code, _h_data in portfolio.holdings.items():
            _h_data.pop("_dd_reduced_today", None)
        ...
        for code in list(portfolio.holdings.keys()):
            ...
            if not should_sell and cum_return > tp_pct and code in portfolio.holdings:
            ...
            if _step_tp and not should_sell and cum_return > 20 and code in portfolio.holdings:
            ...
            if _rsi_sell_threshold > 0 and not should_sell and cum_return > 10 and code in portfolio.holdings:
                try:
            ...
            if _mom_decay_days > 0 and code in portfolio.holdings and cum_return > 5:
                ...
                if len(_mom_hist) >= _mom_decay_days:
                    ...
                    if _prev_avg > 0 and _recent_avg < _prev_avg * 0.8:  # 动量下降20%
                        should_sell = True
