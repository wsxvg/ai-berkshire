...
# -*- coding: utf-8 -*-
"""
R29 严格 OOS — 回调深度分层 (深跌 vs 浅跌信号给不同 boost)
================================================================
R24-R28 结论: WIDE配置(回调-15~-2% + amount top3 + boost0.6/0.4/0.2)是信号甜点位,
  回调窗口/boost/线路/topN/动量门控/信号有效期 6 维度全验证到头 (最优 ~+0.86%)。
R29 目标: R24 证明"宽回调最优"(-15~-2%), 但未测回调**深度段**内部谁更强。
  深跌信号(接近 -15%, 大佬敢于逆势重仓抄底, 信息含量高) vs 浅跌信号(接近 -2%, 顺势加仓)
  谁该给更高 boost? 引擎加 cb_band_mode 回调深度分层:
  ci1: cb_band_mode="deep_boost"  深回调(-15~-8%)boost×1.5, 浅回调(-8~-2%)×0.8 → 重仓深跌
  ci2: cb_band_mode="shallow_boost" 深回调×0.8, 浅回调(-8~-2%)×1.5 → 重仓浅跌
  ci3: cb_band_mode="deep_only"   只保留深回调(-15~-10%)boost, 其余归零 → 只做深度抄底
全部基于 WIDE 最优配置 + amount top3 信号。BASELINE(ci0) 复用 R21 strict_test_ci0_wi*.json;
对照 = R24 WIDE(无分层) +0.859%。引擎改动见 backtest/engine/backtest.py 的 cb_band_mode。
========================================================================
"""
...
def _wide_cfg(band_mode, deep_lo=-15, deep_hi=-8, dm=1.5, sm=0.8):
    return {
        ...
        "signal_line": "amount",
        "sm_params": {**WIDE_SM, "cb_band_mode": band_mode,
                      "deep_lo": deep_lo, "deep_hi": deep_hi,
                      "deep_mult": dm, "shallow_mult": sm},
    }
...
CANDIDATES = [
    ...
    # ci1: WIDE + 重仓深跌回调信号
    ("KC_WIDE_DEEP_BOOST", _wide_cfg("deep_boost")),
    # ci2: WIDE + 重仓浅回调信号
    ("KC_WIDE_SHALLOW_BOOST", _wide_cfg("shallow_boost")),
    # ci3: WIDE + 只做深度抄底 (深回调 -15~-10%)
    ("KC_WIDE_DEEP_ONLY", _wide_cfg("deep_only", deep_lo=-15, deep_hi=-10)),
]
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(f"  run <ci> <wi>: 跑单个窗口 (ci=1 deep_boost, ci=2 shallow_boost, ci=3 deep_only)")
        print("  test: 聚合 (自动读 R21 BASELINE + R29 候选)")
        sys.exit(1)
    cmd = sys.argv[1]
    if cmd == "test":
        aggregate_test()
