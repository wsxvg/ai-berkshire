...
| 候选 | 策略 | 配置 |
|---|---|---|
| C_EQW13 | 买入持有 13 类均等 | top_k=13, 不择时 |
| C_TAA_K4 | 动量 top-4 | 月度再平衡 |
| C_TAA_K3 | 动量 top-3（更集中）| 月度再平衡 |
| C_TAA_K5 | 动量 top-5 + 熊市防御 | bear_max_equity=0 |
...
**OOS 风格验证（wi21–23，局部 n=3 烟雾测试，尚未触发 Actions）**：
| 候选 | wi21 | wi22 | wi23 | 备注 |
|---|---|---|---|---|
| C_EQW13 | -7.173%（bench +13.8%）| +1.668%（bench -11.3%）| +13.010%（bench +28.6%）| |
| C_TAA_K4 | +16.679%（b +13.8%）| +9.968%（b -11.3%）| -16.772%（b +28.6%）| 与基准差异大 |
| C_TAA_K3 | +23.422%（b +13.8%）| +1.671%（b -11.3%）| -14.603%（b +28.6%）| |
| C_TAA_K5 | -3.175%（b +13.8%）| （运行中）| | bear 门槛可能过于严格 |

**观察**：熊市期（wi22，CSI300 -11%）动量轮动跑赢基准（+9-16% vs -11%），但牛市期（wi23，CSI +28%）被基准大幅甩开 → **熊市防守有效，牛市追高风险大**。C_EQW13 长期锚最稳但波动不低（dd 35%）。待完整 59 窗汇总后判断。
