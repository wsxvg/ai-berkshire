...
**真实年化（full-span CAGR，2016-12 → 2026-08，9.65 年一次性复利）**：
| 候选 | total_return | **真实 CAGR** |
|---|---|
| CSI300 | +438.6% | **+19.06%** |
| **C_TAA_K3** | +418.8% | **+18.60%** |
| C_TAA_K4 | +338.0% | +16.54% |
| C_TAA_K5 | +216.2% | +12.67% |
| C_EQW13 | +150.3% | +9.97% |

**⚠️ walk-forward 每窗平均严重失真**：59 个重叠 90 天窗算术平均无法年化（重叠 50%→ 利润被重复计算）。原始 avg/w 显示 "EQW13 最优" 是统计幻觉，真实 CAGR 排名完全相反。

**核心结论**：
1. **动量 top-3 (K3) 复制了 CSI300 的收益**：18.60% vs 19.06% CAGR，alpha ≈ -0.46% ≈ 零 — **本质是"用 13 个月换仓模拟沪深300"**，无超额收益
2. **K4/K5 vs K3**：K3 比 K4 多 +2.06% CAGR，比 K5 多 +5.93% — 熊防御 (K5) 确实有害
3. **EQW13 真实年化最低 +9.97%**：买入持有 13 类等权在过去 9.65 年跑输 CSI300 超 9 个百分点 — 分散化到黄金/债券/海外稀释了 A 股超额
4. **跨资产动量无法战胜最简单的宽基指数**：K3 的努力（月度调仓、动量择时）最终只换来跟 CSI300 一样的结果，但换仓成本 + 摩擦更高

**累计证伪记录**：R23-R32 多因子信号证伪 → R33 克隆模式证伪 → **R34 跨资产动量轮动证伪（K3 tracks CSI300, no alpha）**。
