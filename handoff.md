...
**策略概念**：跨 13 类宽基资产（A股大小盘、红利低波、纳指/标普QDII、恒生、国债、信用债、黄金、货币）动量轮动。月度再平衡，CSI300 200日均线择时控制仓位，等权 top-K。

**🚨 重大数据 Bug 修复（commit fb37d59）**：
发现并修复了**数据源层次**的严重 bug：
- **问题**：京东金融 API 返回的 `yAxis = (NAV_t / base_NAV - 1) × 100`（累计收益率%），不是单位净值。引擎原代码把 yAxis 当价格直接用于所有的 NAV 计算
- **连锁后果**：
  1. 负 yAxis 的亏损数据被非法过滤（原 `if v > 0`），2016-2024 熊市期关键数据丢失
  2. 负 yAxis 导致份额计算 `alloc / yAxis` 出错（分母负/零 → 数据异常）
  3. 所有收益率、CAGR、动量分数基于错误数值，年化 18-19% 是假象
- **修复**：`load_navseries()` 转换 `norm_NAV = 1 + yAxis/100`（归一化 NAV，base=1.0），下游收益率计算全部恢复真实
- **验证**：修复后 CSI300 真实 CAGR = **+5.83%**（合理：2017-2026 A 股宽基全收益 5-7%），修复前 19% 是偏高假象

Engine-level bug fixes（commit 60c1656）：
- `if len(dates) < rebalance_days` → `if len(dates) < 2`（C_EQW13 的 `rebalance_days=99999` 原来永远返回 None）
- 删除 `and i > 126` 暖up 守卫（60 交易日窗内零交易）

**OOS 完整结果（59 test windows, full-span CAGR）**：
| 候选 | avg/w | 真实 CAGR | beats | vs CSI300 |
|---|---|---|---|---|
| **C_EQW13** 买持分散 | +1.30% | **+6.93%** | 28/59 | **+1.10%** |
| CSI300 基准 | — | +5.83% | — | — |
| C_TAA_K4 动量 top-4 | +1.12% | +0.84% | 26/59 | **-5.0%** |
| C_TAA_K3 动量 top-3 | +1.01% | +0.41% | 25/59 | **-5.4%** |
| C_TAA_K5 +熊防御 | +1.05% | +0.33% | 26/59 | **-5.5%** |

**分行情拆解**：
| 行情 | 窗数 | EQW13 (avg/w) | K3 (avg/w) | CSI300 (avg/w) |
|---|---|---|---|---|
| 牛市 | 35 | +3.19% | +2.17% | +7.06% |
| 熊市 | 24 | -1.46% | -0.69% | -5.54% |

**核心结论**：
1. **动量轮动全面崩盘（真实 CAGR < 1%）**：月度再平衡 + 跨资产追涨杀跌反复磨损，月均仅 5-10bps，远不足以弥补换仓摩擦
2. **牛市被大盘甩开（CSI300 +7% vs 动量 +2%/窗）**：动量追错方向，月度再平衡在趋势市场中不断高买低卖
3. **分散化 EQW13 胜出（+6.93% CAGR）**：纳指/标普 QDII 在 2017-2026 期间实现了约 8-10x 的增长，超额收益覆盖了 A 股弱势
4. **熊市保护有效但不足以弥补差距**：策略在熊市少跌 4-5 个百分点，但月复一月的微弱动量收益无法在牛市追回 5% 差距

**累计证伪记录**：R23-R32 多因子信号 → R33 克隆模式 → **R34 跨资产动量轮动证伪（年化 < 1%，熊市保护赢不了牛市差距）**。

### R35 — 多引擎 yAxis 数据一致性审计 + 实盘 cashflow bug（2026-08-12）

**背景**：用户手工核对易方达纳斯达克(110026)和Z先生养基实盘的年化数据，质疑我们数据源是否有严重 bug。逐项审计结果如下：

#### 1. 110026 标准基金 — 手工 API 精确验证通过

| 日期 | 复权净值 | yAxis 累计% |
|---|---|---|
| 2025-08-11 | 2.4526 | +145.26% |
| 2026-08-11 | 3.6366 | +263.66% |

`(3.6366/2.4526 - 1) = 48.28%`，**和 JD APP 显示的近一年涨幅 48.28% 完全匹配**。`yAxis = (NAV/base_NAV - 1)*100`，线性变换、无精度损失。之前我报的 51% 是起点日期偏了（用了 2025-08-08 而非精确一年），不是数据源 bug。

#### 2. 各引擎 yAxis 处理审计

| 引擎 | 处理方式 | 是否正确 |
|---|---|---|
| `backtest.py`（主引擎，R23-R32 大佬策略） | `values = 100 + yAxis`，线性 base=100 空间 | ✅ 正确 — 收益率比值计算时缩放抵消 |
| `alloc_backtest.py`（TAA，R34） | 原代码 `if v > 0: series=yAxis` 当价格用 | ❌ 已修复（commit fb37d59） |
| `fundamental_momentum.py` | `ret = (cur - yaxis[-21]) / (abs(yaxis[-21])/100 + 0.01)` 混用空间 | ⚠️ 有 bug，但**从未被 import 调用** |
| `speedup.py` | `(navs[i] - 1)*100` 正向格式化 | ✅ 为产出端转换 |

**重要结论**：
- **大佬策略（R23-R33）yAxis 处理没有问题**，所有 CAGR 数字可信
- 只有 R34（alloc_backtest.py）受灾，已修复

#### 3. Z先生养基 实盘 cashflow bug（核心发现）

`data/player_returns_ranked_real.json` 缓存数据：
| 字段 | 值 |
|---|---|
| uid | 14345330 |
| 总投入 | 22,200 |
...
JD APP 公开页显示 ~30%+。差距原因：
- 实盘有**中途存取**（赎回了 54% 本金），不是静态买入持有
- 我们的 cache 使用 **DWR（金额加权）** 算法：`total_ret = (end_val - invested) / invested`，被巨额赎回稀释
- JD APP 显示 **TWR（时间加权）**：按每日仓位 × 每日收益率复利链接，不受存取影响
- **TWR ≠ DWR 的数学本质**：TWR 反映真实投资能力，DWR 反映"口袋里实际赚了多少"——被 cashflow 严重扭曲
- Z先生实盘当前重仓纳指/科技，近一年涨幅 30%+（和他实际看对标的的 48% 中幅度跟涨）一致

**可行的修复路径**：需要每日逐日持仓快照（而不是 trades 汇总），用 Modified Dietz 或每日链接法重构 TWR。JD 公开 API（`getFundDetailPageInfo`、`getPageMutilData`）当前全部返回 404/空，说明需要浏览器 cookies 鉴权或接口已下线。现在暂不修复。

#### 4. 数据源质量分级

| 数据类型 | 状态 | 备注 |
|---|---|---|
| 标准基金 NAV/净值 | ✅ 可信 | yAxis = 线性变换，110026 手工验证 48.28% 精确匹配 |
| 大佬交易记录 v7 (550k 条) | ✅ 可信 | 2023-08-08 ~ 2026-08-08 |
| 实盘 TWR 年化 | ❌ **不可信** | cache 是 DWR 汇总，API 不可用，需要重构 |
